...
Sentence-Transformers sidecar used by garfield-router's graphify pre-stage. Stateless HTTP. One model loaded at startup; reload by restarting the container with a different `EMBEDDING_MODEL`.
...
```bash
cd garfield-router/embedding-service
...
Built and run as part of the garfield-router stack via `docker-compose`. Standalone:

```bash
cd garfield-router/embedding-service
docker build -t garfield/embedding-service .
docker run --rm -p 8053:8053 garfield/embedding-service
```
